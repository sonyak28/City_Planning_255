...
census_final = pd.read_csv('../data/census_tract_data_2024.csv')
...
stations_final.to_csv("../data/stations_with_correct_census.csv", index=False)
...
for name in test_stations:
    match = stations_final[stations_final['name'].str.contains(name, na=False)]
    if len(match) > 0:
        row = match.iloc[0]
